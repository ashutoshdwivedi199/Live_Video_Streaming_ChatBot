...
- Robust in handling local failures like camera disconnection and GStreamer crashes
- Network outage detection via HTTP server health check
- Auto-restart of HTTP server if unreachable
- Segment delivery monitoring
- Client-side fallback awareness (via chatbot or logs)
- HTTP server watchdog thread to monitor network health with retry, logging, and alert hook
...
- Multi-Camera Support: Launch multiple camera streams in parallel using threads. If one camera fails, others continue streaming.
- Camera Retry Logic: If a camera fails to return frames, the system waits and retries instead of crashing out.
- GStreamer Auto-Restart: Automatically restarts the GStreamer pipeline if it crashes or the pipe breaks.
- AI Fallback Control: Users can switch between AI-enhanced and regular streaming modes via chatbot interface if AI processing causes lag or buffering.
- HTTP server watchdog thread to monitor network health with retry, logging, and alert hook
...
## How to Run
python app.py :: so that user can control basic streaming/enhanced streaming based on latency with default config parameters.
python main.py :: user can choose config parameters based on the need.
### 1. Basic Streaming (no detection)
python start_stream.py :: if user only wants basic streaming.

### 2. Streaming with Object Detection
python object_stream.py :: if user only wants enhanced streaming.
